...
namespace Giis.Qacover.Model
{
	...
	public class QueryKey
	{
		...
		private string GetSqlHash(string sql)
		{
			...
			{
				return JavaCs.GetHashMd5(sql);
			}
			else
			{
				//standard sha256 hash not available in Java 1.4
		...
		public virtual string GetClassName()
		{
			...
			{
				// remove method.line.sql
				sb.Append(i == 0 ? string.Empty : ".").Append(parts[i]);
			}
			return sb.ToString();
		}
		...
		/// <summary>Gets the method name with the line number (opt in)</summary>
		public virtual string GetMethodName(bool includeLineNumbers)
		{
			string[] parts = JavaCs.SplitByDot(key);
			return parts[parts.Length - 3] + (includeLineNumbers ? ":" + parts[parts.Length - 2] : string.Empty);
		}
